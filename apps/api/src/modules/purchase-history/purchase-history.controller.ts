...
export class PurchaseHistoryController {
  ...
  create(
    ...
    return this.purchaseHistoryService.create(
      user.organizationId,
      user.id,
      dto,
    );
  ...
  bulkCreate(
    ...
    return this.purchaseHistoryService.bulkCreate(
      user.organizationId,
      user.id,
      dto,
    );
  ...
  @ApiOperation({
    summary: "List purchase history with filters and pagination",
  })
  @ApiResponse({
    status: 200,
    description: "Paginated list of purchase history records",
  })
  findAll(
    @CurrentUser() user: ICurrentUser,
  ...
  @ApiOperation({
    summary: "Get purchase statistics (totals, by supplier, by product)",
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot update non-PENDING purchase",
  })
  @ApiResponse({ status: 404, description: "Purchase record not found" })
  update(
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot receive non-PENDING purchase",
  })
  @ApiResponse({ status: 404, description: "Purchase record not found" })
  receive(
    @Param("id", ParseUUIDPipe) id: string,
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot return non-RECEIVED purchase",
  })
  @ApiResponse({ status: 404, description: "Purchase record not found" })
  returnPurchase(
    @Param("id", ParseUUIDPipe) id: string,
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN, UserRole.MANAGER)
  @ApiOperation({
    summary: "Soft delete a purchase (only PENDING or CANCELLED)",
  })
  @ApiParam({ name: "id", type: "string", format: "uuid" })
  @ApiResponse({
    status: 204,

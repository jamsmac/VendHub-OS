    ...
    switch (type) {
      case RecommendationType.MACHINE:
        if (!machineId) {
          ...
          recommendations =
            await this.recommendationsService.getMachineRecommendations(
              machineId,
              user.organizationId,
              user.id,
              limit,
      ...
      case RecommendationType.SIMILAR:
        if (!productId) {
          ...
          recommendations =
            await this.recommendationsService.getSimilarProducts(
              productId,
              user.organizationId,
              limit,
            );
      ...
      case RecommendationType.COMPLEMENTARY:
        if (!productId) {
          ...
          recommendations =
            await this.recommendationsService.getComplementaryProducts(
              productId,
              user.organizationId,
              limit,
            );
      ...
      case RecommendationType.TIME_BASED:
        recommendations =
          await this.recommendationsService.getTimeBasedRecommendations(
            user.organizationId,
            undefined,
            limit,
          );

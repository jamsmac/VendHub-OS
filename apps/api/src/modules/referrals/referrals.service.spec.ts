...
describe("ReferralsService", () => {
  ...
  describe("getReferralCode", () => {
    ...
    it("should throw NotFoundException when user not found", async () => {
      ...
      await expect(service.getReferralCode("non-existent")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("regenerateReferralCode", () => {
    ...
    it("should use custom code when provided and available", async () => {
      ...
      userRepo
        .findOne!.mockResolvedValueOnce(user) // find user
    ...
    it("should throw NotFoundException when user not found", async () => {
      ...
      await expect(
        service.regenerateReferralCode("non-existent"),
  ...
  describe("applyReferralCode", () => {
    ...
    it("should apply referral code and award welcome bonus", async () => {
      userRepo
        .findOne!.mockResolvedValueOnce(referredUser)
  ...
  describe("getReferralSummary", () => {
    ...
    it("should throw NotFoundException when user not found", async () => {
      ...
      await expect(service.getReferralSummary("non-existent")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("getUserReferrals", () => {
    it("should return paginated referral list", async () => {
      ...
      const result = await service.getUserReferrals("u-1", {
        page: 1,
        limit: 20,
      });
  ...
  describe("generateUniqueCode (via regenerateReferralCode)", () => {
    it("should generate an 8-character code", async () => {
      const user = { id: "u-1", referralCode: "OLD" };
      userRepo.findOne!.mockResolvedValueOnce(user).mockResolvedValueOnce(null); // code is unique
      userRepo.update!.mockResolvedValue({});

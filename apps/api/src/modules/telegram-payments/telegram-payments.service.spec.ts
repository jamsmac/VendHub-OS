...
describe("TelegramPaymentsService", () => {
  ...
  describe("handleSuccessfulPayment", () => {
    ...
    it("should create new payment record when no pending payment found", async () => {
      ...
      await service.handleSuccessfulPayment(123456789, {
        invoicePayload: JSON.stringify({
          orderId,
          userId,
          organizationId: orgId,
  ...
  describe("getPayment", () => {
    ...
    it("should throw NotFoundException when payment not found", async () => {
      ...
      await expect(service.getPayment("bad", orgId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("getStats", () => {
    it("should return aggregated payment statistics", async () => {
      ...
      qb.getMany.mockResolvedValue([
        ...
        {
          status: TelegramPaymentStatus.FAILED,
          amount: 10000,
          provider: TelegramPaymentProvider.CLICK,
          refundedAmount: null,
        },
        {
          status: TelegramPaymentStatus.REFUNDED,
          amount: 20000,
          provider: TelegramPaymentProvider.PAYME,

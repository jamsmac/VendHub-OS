...
describe("FavoritesService", () => {
  ...
  describe("removeFavorite", () => {
    it("should remove a favorite by id", async () => {
      ...
      expect(favoriteRepo.softDelete).toHaveBeenCalledWith({
        id: favoriteId,
        userId,
      });
    });
  ...
  describe("addFavoritesBulk", () => {
    ...
    it("should handle errors gracefully in bulk add", async () => {
      const dto = {
        items: [{ type: FavoriteType.PRODUCT, productId: "non-existent" }],
      };
  ...
  describe("getFavorites", () => {
    ...
    it("should filter by type when specified", async () => {
      ...
      favoriteRepo.count.mockResolvedValueOnce(1).mockResolvedValueOnce(0);

...
describe("MaterialRequestsService", () => {
  ...
  const _mockDeliveredRequest: Partial<MaterialRequest> = {
    ...mockRequest,
    id: "req-uuid-6",
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(MaterialRequestItem),
          useValue: itemRepo,
        },
        {
          provide: getRepositoryToken(MaterialRequestHistory),
  ...
  describe("createRequest", () => {
    ...
    it("should calculate totalAmount from items", async () => {
      ...
      requestRepo.save!.mockImplementation((entity) =>
        Promise.resolve({
          ...entity,
  ...
  describe("sendToSupplier", () => {
    it("should transition APPROVED -> SENT", async () => {
      ...
      const result = await service.sendToSupplier(
        "req-uuid-3",
        USER_ID,
        ORG_ID,
      );
  ...
  describe("getRequestHistory", () => {
    it("should return history for a valid request", async () => {
      ...
      historyRepo.find!.mockResolvedValue([
        {
          fromStatus: MaterialRequestStatus.DRAFT,
          toStatus: MaterialRequestStatus.NEW,
        },
      ]);

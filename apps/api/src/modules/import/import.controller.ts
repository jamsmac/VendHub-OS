...
export class ImportController {
  ...
  @ApiResponse({
    status: 200,
    description: "Paginated list of import sessions",
  })
  async listSessions(
    @CurrentOrganizationId() organizationId: string,
  ...
  @ApiResponse({
    status: 200,
    description:
      "Classification result with column mapping and confidence score",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Session submitted for approval or auto-approved",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Session approved and optionally executed",
  })
  ...
  @ApiQuery({
    name: "actionType",
    enum: ["insert", "update", "merge", "skip", "delete", "restore"],
    required: false,
  })
  ...
  @Roles("manager", "admin", "owner")
  @ApiOperation({
    summary: "List schema definitions for import auto-classification",
  })
  @ApiQuery({ name: "domain", enum: DomainType, required: false })
  @ApiResponse({ status: 200, description: "List of schema definitions" })
  async listSchemaDefinitions(@Query("domain") domain?: DomainType) {
  ...
  async listValidationRules(@Query("domain") domain?: DomainType) {
  ...
  async getSampleStructure(@Param("importType") importType: ImportType) {
    ...
    > = {
      ...
      [ImportType.TRANSACTIONS]: {
        columns: ["transactionDate", "type", "amount", "description"],
        sampleRow: {},
      },
      [ImportType.INVENTORY]: {
        columns: ["productSku", "quantity", "location"],
        sampleRow: {},
      },
      [ImportType.CUSTOMERS]: {
        columns: ["phone", "email", "firstName", "lastName"],
        sampleRow: {},
      },
      [ImportType.PRICES]: {
        columns: ["productSku", "price", "effectiveDate"],
        sampleRow: {},
      },
      [ImportType.CATEGORIES]: {
        columns: ["name", "nameUz", "parentCategory", "sortOrder"],
        sampleRow: {},
      },
      [ImportType.LOCATIONS]: {
        columns: ["name", "address", "city", "latitude", "longitude"],
        sampleRow: {},
      },

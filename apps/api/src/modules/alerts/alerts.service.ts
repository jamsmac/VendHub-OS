...
export class AlertsService {
  ...
  async createRule(
    ...
    if (dto.condition === AlertCondition.BETWEEN && dto.thresholdMax == null) {
      throw new BadRequestException(
        "thresholdMax is required for BETWEEN condition",
      );
    }
  ...
  async findAllRules(
    ...
    if (machineId) {
      qb.andWhere("(r.machineId = :machineId OR r.machineId IS NULL)", {
        machineId,
      });
    }
    if (search) {
  ...
  async triggerAlert(
    ...
    const alertHistory = this.historyRepository.create({
      ...
      message:
        message ||
        `${rule.name}: value ${value} ${rule.condition} ${rule.threshold}`,
    });
  ...
  async acknowledgeAlert(
    ...
    if (alert.status !== AlertHistoryStatus.ACTIVE) {
      throw new BadRequestException(
        `Cannot acknowledge alert in ${alert.status} status`,
      );
    }
  ...
  async resolveAlert(
    ...
    if (
      ![AlertHistoryStatus.ACTIVE, AlertHistoryStatus.ACKNOWLEDGED].includes(
        alert.status,
      )
    ) {
      throw new BadRequestException(
        `Cannot resolve alert in ${alert.status} status`,
      );
    }
  ...
  async dismissAlert(
    ...
    if (
      ...
      throw new BadRequestException(
        `Cannot dismiss alert in ${alert.status} status`,
      );
    }
  ...
  async getAlertHistory(
    ...
    if (startDate && endDate) {
      qb.andWhere("h.triggeredAt BETWEEN :startDate AND :endDate", {
        startDate,
        endDate,
      });
    }

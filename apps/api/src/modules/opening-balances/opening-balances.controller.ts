...
export class OpeningBalancesController {
  ...
  create(
    ...
    return this.openingBalancesService.create(
      user.organizationId,
      user.id,
      dto,
    );
  ...
  @ApiResponse({
    status: 201,
    description: "Opening balances created successfully",
  })
  bulkCreate(
    @CurrentUser() user: ICurrentUser,
    @Body() dto: BulkCreateOpeningBalanceDto,
  ) {
    return this.openingBalancesService.bulkCreate(
      user.organizationId,
      user.id,
      dto,
    );
  ...
  @ApiOperation({
    summary: "List opening balances with filters and pagination",
  })
  @ApiResponse({
    status: 200,
    description: "Paginated list of opening balances",
  })
  findAll(
    @CurrentUser() user: ICurrentUser,
  ...
  @ApiOperation({
    summary: "Update an opening balance (only unapplied records)",
  })
  @ApiParam({ name: "id", type: "string", format: "uuid" })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 200,
    description: "Opening balance applied successfully",
  })
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN, UserRole.MANAGER, UserRole.ACCOUNTANT)
  @ApiOperation({
    summary: "Apply all unapplied opening balances for a specific date",
  })
  @ApiResponse({
    status: 200,
    description: "Opening balances applied successfully",
  })
  applyAll(@CurrentUser() user: ICurrentUser, @Body() dto: ApplyAllDto) {
    return this.openingBalancesService.applyAll(
      user.organizationId,
      user.id,
      dto,
    );
  ...
  @ApiOperation({
    summary: "Soft delete an opening balance (only unapplied records)",
  })
  @ApiParam({ name: "id", type: "string", format: "uuid" })
  @ApiResponse({
    status: 204,
    description: "Opening balance deleted successfully",
  })

...
export class AiController {
  ...
  async importFromImage(@Body() dto: ImportFromImageDto) {
    ...
    return this.aiService.parseProductsFromImage(dto.image_base64, dto.context);
  }
  ...
  async detectSalesAnomaly(@Body() dto: DetectAnomalyDto) {
    return this.aiService.detectSalesAnomaly(
      dto.machine_id,
      dto.sales_data.map((d) => ({
        date: d.date,
        amount: d.amount,
        transactions: d.transactions,
      })),
      {
        avgAmount: dto.historical_average.avg_amount,
        avgTransactions: dto.historical_average.avg_transactions,
      },
    );
  }
  ...
  async batchCategorize(@Body() dto: BatchCategorizeDto) {
    const results = await this.aiService.batchCategorize(dto.products);

...
Return only the response text.`;
  ...
  async detectSalesAnomaly(
    ...
    const avgTx =
      recentData.reduce((sum, d) => sum + d.transactions, 0) /
      recentData.length;
...
Return JSON:
{
  ...
  private getMockResponse(prompt: string): any {
    ...
    // Check for categorization - must have "Categorize"
    if (prompt.includes("Categorize")) {
      return { category: "snack", subcategory: "chocolate", confidence: 0.8 };
    }
    // Check for product extraction - "Extract product" or import-related
    if (
      prompt.includes("Extract product") ||
      prompt.includes("extract product")
    ) {
      return {
        products: [

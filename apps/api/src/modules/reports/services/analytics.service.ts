...
export class AnalyticsService {
  ...
  async getSnapshots(
    ...
    qb.orderBy("snapshot.snapshotDate", "DESC").skip(skip).take(limit);
  ...
  async getSnapshot(id: string): Promise<AnalyticsSnapshot> {
    ...
    if (!snapshot) {
      throw new NotFoundException(
        `Analytics snapshot with ID "${id}" not found`,
      );
    }
  ...
  async createDailySnapshot(
    ...
    const snapshotData: Partial<AnalyticsSnapshot> = {
      ...
      averageTransactionValue: parseFloat(
        transactionAgg?.averageTransactionValue ?? "0",
      ),
      stockRefills: parseInt(taskAgg?.stockRefills ?? "0", 10),
      maintenanceTasksCompleted: parseInt(
        taskAgg?.maintenanceTasksCompleted ?? "0",
        10,
      ),
  ...
  async createWeeklySnapshot(
    ...
    const agg = await this.snapshotRepo
      ...
      .andWhere("s.snapshotDate >= :start", {
        start: this.toDateString(weekStart),
      })
    ...
    const snapshotData: Partial<AnalyticsSnapshot> = {
      ...
      maintenanceTasksCompleted: parseInt(
        agg?.maintenanceTasksCompleted ?? "0",
        10,
      ),
      ...
      metadata: {
        ...
        weekStart: this.toDateString(weekStart),
        weekEnd: this.toDateString(weekEnd),
      },
    };
  ...
  async createMonthlySnapshot(
    ...
    const agg = await this.snapshotRepo
      ...
      .andWhere("s.snapshotType = :type", { type: SnapshotType.DAILY })
      .andWhere("s.snapshotDate >= :start", {
        start: this.toDateString(monthStart),
      })
    ...
    const profitMarginPct =
      totalRevenue > 0
        ? Math.round(
            ((totalRevenue - operationalCosts) / totalRevenue) * 10000,
          ) / 100
        : 0;
  ...
  async updateDailyStats(
    ...
    const statsData: Partial<DailyStats> = {
      ...
      // Machines
      activeMachinesCount: parseInt(
        activeMachinesResult?.activeMachinesCount ?? "0",
        10,
      ),
      ...
      collectionTasksCompleted: parseInt(
        taskAgg?.collectionTasksCompleted ?? "0",
        10,
      ),
      cleaningTasksCompleted: parseInt(
        taskAgg?.cleaningTasksCompleted ?? "0",
        10,
      ),
  ...
  // ============================================================================
  // DASHBOARD (delegated to DashboardAnalyticsService)
  // ============================================================================

  /**
   * Get dashboard data for the organization.
   * Delegates to DashboardAnalyticsService.
   */
  async getDashboardData(organizationId: string) {
    return this.dashboardAnalyticsService.getDashboardData(organizationId);
  }

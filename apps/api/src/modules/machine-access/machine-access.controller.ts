...
export class MachineAccessController {
  ...
  async getAccessByMachine(
    ...
    return this.machineAccessService.getAccessByMachine(
      machineId,
      orgId,
      includeInactive,
    );
  ...
  async getAccessByUser(
    ...
    return this.machineAccessService.getAccessByUser(
      userId,
      orgId,
      includeInactive,
    );
  ...
  @ApiOperation({
    summary: "Apply a template to grant access for users to a machine",
  })
  @ApiResponse({
    status: 201,
    description: "Template applied, access records created",
  })
  @Roles("owner", "admin", "manager")
  @HttpCode(HttpStatus.CREATED)
  async applyTemplate(

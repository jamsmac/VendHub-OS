...
export class TasksController {
  ...
  @ApiResponse({
    status: 200,
    description: "Kanban board with tasks grouped by status",
  })
  getKanbanBoard(
  ...
  getMyTasks(@CurrentUser() user: ICurrentUser) {
  ...
  @ApiQuery({
    name: "search",
    required: false,
    description: "Search by task number or description",
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Photo not expected in current status",
  })
  @ApiResponse({ status: 404, description: "Task not found" })
  uploadPhotoBefore(
    @Param("id", ParseUUIDPipe) id: string,
  ...
  @ApiResponse({
    status: 200,
    description: "Photo uploaded and task completed",
  })

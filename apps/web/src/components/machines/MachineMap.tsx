...
> = {
  ...
  error: { label: "Error", color: "text-red-600", markerColor: "bg-red-500" },
  maintenance: {
    label: "Maintenance",
    color: "text-blue-600",
...
export function MachineMap({ machines }: MachineMapProps) {
  const [selectedMachine, setSelectedMachine] = useState<MapMachine | null>(
    null,
  );
        ...
        <div className="relative bg-slate-100 h-[500px] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {machines.length} machines on map
            </p>
            <p className="text-xs text-muted-foreground mt-2">
              Yandex Maps integration will be added after package installation
            </p>
          </div>
        </div>
      </Card>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${status.markerColor}`}
                    />

...
export function InventoryScreen() {
  ...
  const {
    ...
    queryFn: () =>
      inventoryApi.getOperator().then((res) => res.data as InventoryItem[]),
  ...
  const {
    ...
    queryFn: () =>
      inventoryApi.getMovements().then((res) => res.data as Movement[]),
    enabled: activeTab === "movements",
  });

  const isLoading =
    activeTab === "operator" ? loadingOperator : loadingMovements;
              ...
              <View style={styles.summaryRow}>
                ...
                <View style={styles.summaryCard}>
                  <Text style={[styles.summaryValue, { color: "#EF4444" }]}>
                    {
                      operatorInventory.filter(
                        (i) => i.status === "critical" || i.status === "empty",
                      ).length
                    }
                  </Text>
                  <Text style={styles.summaryLabel}>

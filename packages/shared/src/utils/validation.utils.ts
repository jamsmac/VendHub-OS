...
export function validateEmail(email: string): ValidationResult {
  ...
  if (!email) {
    ...
    errors.push(
      `Email слишком длинный (максимум ${STRING_LENGTHS.EMAIL_MAX} символов)`,
    );
  }
...
export function validateSku(sku: string): ValidationResult {
  ...
  if (!sku) {
    ...
    errors.push(
      `SKU слишком короткий (минимум ${STRING_LENGTHS.SKU_MIN} символа)`,
    );
  } else if (sku.length > STRING_LENGTHS.SKU_MAX) {
    errors.push(
      `SKU слишком длинный (максимум ${STRING_LENGTHS.SKU_MAX} символов)`,
    );
...
export function validateQuantity(quantity: number): ValidationResult {
  ...
  if (quantity === undefined || quantity === null) {
    ...
    errors.push(
      `Количество слишком большое (максимум ${NUMBER_LIMITS.QUANTITY_MAX})`,
    );
  }
...
export function validateSlug(slug: string): ValidationResult {
  ...
  if (!slug) {
    ...
    errors.push(
      `Slug слишком короткий (минимум ${STRING_LENGTHS.SLUG_MIN} символа)`,
    );
  } else if (slug.length > STRING_LENGTHS.SLUG_MAX) {
    errors.push(
      `Slug слишком длинный (максимум ${STRING_LENGTHS.SLUG_MAX} символов)`,
    );
